...
async generarPDFGeneral(req, res) {
  try {
    const servidores = await prisma.servidores.findMany({
      include: { stock_equipos: { include: { tipo_equipo: true } }, sede: true }
    });
    ...
    doc.fontSize(20).text('REPORTE DE SERVIDORES', 100, 100);
    doc.fontSize(12).text(`Total: ${servidores.length} servidores`, 100, 150);
