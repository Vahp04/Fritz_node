...
class PDFKitGenerator {
  ...
  static generateStyledPDF(doc, data) {
    ...
    if (data.usuarios && data.usuarios.length > 0) {
      ...
      const colWidths = [90, 50, 100, 60, 50, 70, 25, 25, 50];
      ...
      data.usuarios.forEach((usuario, index) => {
        ...
        const rowData = [
          ...
          (usuario.cargo || '').substring(0, 10),
          (usuario.correo || '').substring(0, 25),
          `${usuario.rdpfis || ''}/n ${usuario.rdpfin || ''}`.substring(0, 16),
          (usuario.sede?.nombre || 'N/A').substring(0, 12),
          (usuario.departamento?.nombre || 'N/A').substring(0, 17),
